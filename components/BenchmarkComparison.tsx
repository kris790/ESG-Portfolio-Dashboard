...
import { SP500_BENCHMARK, ESGU_BENCHMARK, SUSA_BENCHMARK } from '../constants';
...
const BenchmarkComparison: React.FC<BenchmarkComparisonProps> = ({ portfolioScores }) => {
  const comparisonData = [
    ...
    },
    {
      ...
      susa: SUSA_BENCHMARK.environmentScore,
    },
    {
      metric: 'Social Risk',
      ...
      susa: SUSA_BENCHMARK.socialScore,
    },
    {
      ...
      susa: SUSA_BENCHMARK.governanceScore,
    },
  ];
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">SUSA ETF</th>
            </tr>
          </thead>
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {comparisonData.map((row) => (
              <tr key={row.metric}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{row.susa.toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
       <p className="text-xs text-gray-400 dark:text-gray-500 mt-3">ESGU: iShares ESG Aware MSCI USA ETF. SUSA: iShares MSCI USA ESG Select ETF. Lower scores are better.</p>
    </div>
  );
};
